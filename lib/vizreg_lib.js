...
async function logTestAll(update){
  let testCurr = 0,
      testsPassed = 0,
...
module.exports = async function({ testName=undefined, update=false } = {}){
  checkDirsExist();
  ...
  (async () => {
    ...
    if(testData.numTotalTests == 0) {
      console.log("No visual regression tests were found. Make sure your test files are in a folder named 'viz-tests' located in the root directory of your application.");
    }else if(typeof testName === 'undefined') { // run every test
      let {testsPassed, testsTotal} = await logTestAll(update);
      testsPassedArg = testsPassed;
      testsTotalArg = testsTotal;
    } else {                                    // run specified test
      // let oneTestPassed = await logTestOne(testName, update);
